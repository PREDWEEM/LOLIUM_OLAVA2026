...
# ---------------------------------------------------------
def optimizar_parametros_hidricos_3d(df_meteo, df_campo, modelo_ann, latitud_olava=-36.8799, rango_ventanas=[7]):
    df = df_meteo.copy()
...
st.title("🌾 PREDWEEM by GUILLERMO R. CHANTRE — OLAVARRÍA (BA) lat=-36.8799 lon=-60.2160")
    ...
    if st.button("Ejecutar Barrido 3D"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            ...
            with st.spinner(f'Ejecutando iteraciones 3D en ventanas: {ventanas_a_probar}...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_3d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_olava=-36.8799, rango_ventanas=ventanas_a_probar)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # Bypass Ruptura Temprana (Ajuste específico Olavarría = 0.75)
    ...
    st.sidebar.download_button("📥 Descargar Reporte Completo", output.getvalue(), "PREDWEEM_Integral_Olava_vK4_9_15_UX.xlsx")
