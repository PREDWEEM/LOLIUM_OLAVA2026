# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 PREDWEEM INTEGRAL vK4.9.15 — LOLIUM OLAVARRÍA 2026
...
def load_data(file_uploader, default_name):
    ...
    github_url = f"https://raw.githubusercontent.com/PREDWEEM/LOLIUM_OLAVA2026/main/{default_name}.csv"
    try:
...
# ---------------------------------------------------------
def optimizar_parametros_hidricos_2d(df_meteo, df_campo, modelo_ann, latitud_olava=-36.8799):
    df = df_meteo.copy()
    ...
    df["ET0"] = calcular_et0_hargreaves(df["Julian_days"].values, df["TMAX"].values, df["TMIN"].values, latitud=latitud_olava)
    ...
    for w_max in rango_w_max:
        for ke in rango_ke:
            ...
            # Horizonte térmico de 5 días acoplado en el optimizador
    ...
    return df_resultados.sort_values(by=["F1-Score", "NSE (Flujos)"], ascending=[False, False]).reset_index(drop=True)
    ...
    if st.button("Ejecutar Barrido Hídrico"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            with st.spinner('Ejecutando iteraciones hídricas puras sobre fechas reales...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_2d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_olava=-36.8799)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # 2. Bypass Ruptura Temprana (Olavarría = 0.75) - ESTRICTAMENTE LUEGO DEL DÍA 45
    ...
    # 3. Balance Hídrico Superficial (Olavarría)
    ...
    # Sincronización Event-to-Event por Intervalos Reales Variables
    ...
    st.plotly_chart(go.Figure(data=go.Heatmap(z=[df["EMERREL"].values], x=df["Fecha"], y=["Emergencia"], colorscale=colorscale_hard, zmin=0, zmax=1, showscale=False)).update_layout(height=120, margin=dict(t=30, b=0, l=10, r=10), title="Mapa de Riesgo Temporal (Tasa Diaria Olavarría)"), use_container_width=True)
    ...
    with tab1:
        ...
        with col_main:
            ...
            # --- SOMBREADO DINÁMICO VINCULADO EXCLUSIVAMENTE A LAS FECHAS REALES DE MONITOREO ---
            if df_campo is not None:
