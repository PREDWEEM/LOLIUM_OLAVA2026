# -*- coding: utf-8 -*-
"""Cálculo reproducible de métricas de validación para PREDWEEM Olavarría.
...
La frecuencia y el F1 de picos se calculan sobre flujos Event-to-Event.
El desfase del primer pico se calcula con resolución diaria:

    fecha del primer pico diario validado del modelo
    menos
    fecha del primer pico observado.

También se conserva el desfase basado en la fecha final del intervalo como
...
class StreamlitSilencioso(ContextoNulo):
    """Stub mínimo de Streamlit para ejecutar la aplicación sin interfaz."""
    ...
    def columns(
        ...
        **kwargs: Any,
        ...
        cantidad = (
            especificacion
            if isinstance(especificacion, int)
            else len(especificacion)
        )
...
def ejecutar_app(
    base: Path,
    campo_acumulado: bool = CAMPO_ES_ACUMULADO,
) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="predweem_olavarria_validacion_"
    ) as temporal:
        ...
        globales = runpy.run_path(
            str(destino / APP),
            run_name="__predweem_validacion__",
        )

    claves = ("df", "df_campo", "col_fecha", "col_plm2")
    faltantes_globales = [
        clave for clave in claves if clave not in globales
    ]
    if faltantes_globales:
        raise RuntimeError(
            "La aplicación no produjo las variables necesarias: "
            + ", ".join(faltantes_globales)
        )

    campo = globales["df_campo"].copy()
    flujo = globales["col_plm2"]
    campo[flujo] = (
        pd.to_numeric(campo[flujo], errors="coerce")
        .fillna(0)
        .clip(lower=0)
    )
...
def sincronizar_eventos(
    ...
    """Integra EMERREL en cada intervalo real, incluido el primero."""

    sim = diario[["Fecha", "EMERREL"]].copy()
    sim["Fecha"] = pd.to_datetime(sim["Fecha"], errors="coerce")
    sim["EMERREL"] = (
        pd.to_numeric(sim["EMERREL"], errors="coerce")
        .fillna(0)
        .clip(lower=0)
    )
    ...
    obs[fecha_campo] = pd.to_datetime(
        obs[fecha_campo],
        errors="coerce",
    )
    obs[flujo_campo] = (
        pd.to_numeric(obs[flujo_campo], errors="coerce")
        .fillna(0)
        .clip(lower=0)
    )
    obs = (
        obs.dropna(subset=[fecha_campo])
        .sort_values(fecha_campo)
        .reset_index(drop=True)
    )

    if sim.empty or obs.empty:
        raise ValueError(
            "No hay datos suficientes para sincronizar."
        )

    sim = sim[sim["Fecha"] <= obs[fecha_campo].max()].copy()
    ...
    for i, fila in obs.iterrows():
        ...
        inicio = (
            inicio_serie
            if i == 0
            else pd.Timestamp(obs.loc[i - 1, fecha_campo])
        )
        mascara = (
            (sim["Fecha"] > inicio)
            & (sim["Fecha"] <= fin)
    ...
    salida["Campo_Relativo"] = (
        salida["Flujo_Obs_Abs"] / total_obs
        if total_obs > 0
        else 0.0
    )
    salida["Sim_Relativo"] = (
        salida["Flujo_Sim_Abs"] / total_sim
        if total_sim > 0
        else 0.0
    )
    salida["Campo_Acumulado"] = (
        salida["Campo_Relativo"].cumsum()
    )
    salida["Sim_Acumulado"] = (
        salida["Sim_Relativo"].cumsum()
    )
    return salida


def detectar_picos(valores: Iterable[float]) -> list[int]:
    """Detecta máximos locales sobre flujos relativos por intervalo."""

    x = np.nan_to_num(
        np.asarray(list(valores), dtype=float),
        nan=0.0,
    )
    picos: list[int] = []
    ...
    for i, valor in enumerate(x):
        ...
        es_maximo = (
            valor >= izquierda
            and valor >= derecha
            and (valor > izquierda or valor > derecha)
        )
        prominencia = valor - max(izquierda, derecha)

        if (
            es_maximo
            and valor >= UMBRAL_EVENTO
            and prominencia >= PROMINENCIA_PICO
        ):
            picos.append(i)
...
def aparear_picos(
    obs: list[pd.Timestamp],
    sim: list[pd.Timestamp],
    tolerancia: int,
) -> dict[str, Any]:
    ...
    for fecha_obs in obs:
        candidatos = [
            ...
            if abs((sim[i] - fecha_obs).days) <= tolerancia
        ]
        if candidatos:
            mejor = min(
                candidatos,
                key=lambda i: abs((sim[i] - fecha_obs).days),
            )
...
def primera_fecha(
    ...
    candidatas = fechas_limpias[
        valores_limpios >= umbral
    ].dropna()

    return (
        pd.Timestamp(candidatas.iloc[0])
...
def metricas(
    ...
    omisiones_i = int(
        np.sum(eventos_obs & ~eventos_sim)
    )
    falsos_i = int(
        np.sum(~eventos_obs & eventos_sim)
    )
    ...
    precision_i = (
        hits_i / (hits_i + falsos_i)
        if hits_i + falsos_i
        else 0.0
    )
    sensibilidad_i = (
        hits_i / (hits_i + omisiones_i)
        if hits_i + omisiones_i
        else 0.0
    )
    f1_i = (
        2 * precision_i * sensibilidad_i
        / (precision_i + sensibilidad_i)
        if precision_i + sensibilidad_i
        else 0.0
    )

    indices_obs = detectar_picos(
        sync["Campo_Relativo"]
    )
    indices_sim = detectar_picos(
        sync["Sim_Relativo"]
    )

    fechas_obs = [
        pd.Timestamp(sync.loc[i, "Fecha"])
        for i in indices_obs
    ]
    fechas_sim = [
        pd.Timestamp(sync.loc[i, "Fecha"])
    ...
    tolerancia = max(
        1,
        int(round(sync["Dias_Intervalo"].median())),
    )

    primer_flujo_obs = primera_fecha(
        sync["Fecha"],
        sync["Campo_Relativo"],
        UMBRAL_EVENTO,
    )
    primer_flujo_sim = primera_fecha(
        sync["Fecha"],
        sync["Sim_Relativo"],
        UMBRAL_EVENTO,
    )
    ...
    # Frecuencia y F1: resolución Event-to-Event.
    primer_pico_sim_intervalo = (
        fechas_sim[0]
        if fechas_sim
        else pd.NaT
    )

    # Desfase principal: resolución diaria real del modelo.
    ...
    control = pd.to_datetime(
        globales.get("fecha_control", pd.NaT),
        errors="coerce",
    )
    limite = pd.to_datetime(
        globales.get("fecha_limite", pd.NaT),
        errors="coerce",
    )
    alerta = primera_fecha(
        diario["Fecha"],
        diario["EMERREL"],
        float(globales.get("umbral_er", 0.005)),
    )

    total_campo = float(campo[flujo_campo].sum())
    pec = np.nan
    if total_campo > 0 and pd.notna(control):
        fechas_campo = pd.to_datetime(
            campo[fecha_campo],
            errors="coerce",
        )
        pec = (
    ...
    resultado: dict[str, Any] = {
        ...
        "Intervalo_mediano_dias": float(
            sync["Dias_Intervalo"].median()
        ),
        "Picos_observados": len(fechas_obs),
        "Picos_simulados": len(fechas_sim),
        "Razon_picos_sim_obs": (
            len(fechas_sim) / len(fechas_obs)
            if fechas_obs
            else np.nan
        ),
        ...
        "Delta_primer_flujo_dias": (
            (primer_flujo_sim - primer_flujo_obs).days
            if (
                pd.notna(primer_flujo_obs)
                and pd.notna(primer_flujo_sim)
            )
            else np.nan
        ),
        ...
        "Delta_primer_pico_dias": (
            (primer_pico_sim - primer_pico_obs).days
            if (
                pd.notna(primer_pico_obs)
                and pd.notna(primer_pico_sim)
            )
            else np.nan
        ),
        "Metodo_delta_primer_pico": (
            "pico diario validado simulado - pico observado"
        ),
        ...
        "Objetivo_control_CD": float(
            globales.get("dga_optimo", 600)
        ),
        "Limite_control_CD": float(
            globales.get("dga_critico", 800)
        ),
...
def serializar(valor: Any) -> Any:
    if isinstance(
        ...
        return (
            None
            if pd.isna(valor)
            else pd.Timestamp(valor).isoformat()
        )
...
def guardar(
    ...
    limpio = {
        clave: serializar(valor)
        for clave, valor in resultado.items()
    }

    (
        carpeta / f"metricas_{ID_SITIO}.json"
    ).write_text(
        json.dumps(
            limpio,
            ensure_ascii=False,
    ...
    pd.DataFrame([limpio]).to_csv(
        carpeta / f"metricas_{ID_SITIO}.csv",
        index=False,
        encoding="utf-8-sig",
    )

    sync.to_csv(
        carpeta / f"event_to_event_{ID_SITIO}.csv",
        index=False,
        encoding="utf-8-sig",
        date_format="%Y-%m-%d",
    )
    ...
    columnas = [
        ...
        if columna in diario.columns
    ]
    diario[columnas].to_csv(
        carpeta / f"serie_diaria_{ID_SITIO}.csv",
        index=False,
        encoding="utf-8-sig",
        date_format="%Y-%m-%d",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--campo-acumulado",
        action="store_true",
        default=CAMPO_ES_ACUMULADO,
    )
    args = parser.parse_args()

    base = Path(__file__).resolve().parent
    globales = ejecutar_app(
        base,
        campo_acumulado=args.campo_acumulado,
    )
    sync = sincronizar_eventos(
        globales["df"],
        globales["df_campo"],
        globales["col_fecha"],
        globales["col_plm2"],
